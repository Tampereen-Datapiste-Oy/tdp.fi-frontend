---
path: /it-laitteet/dellpromicro_desktop
thumbnail: /assets/kuva-dell-micro-desktop2-600x450-.jpg
date: 2025-12-18T14:22:00.000Z
head:
  title: Dell Pro Micro pöytäkoneet
  description: Dell Pro Micro -sarja on suunniteltu moderneihin, tilatehokkaisiin
    työympäristöihin. Se on täydellinen valinta avokonttoreihin,
    asiakaspalvelupisteisiin, sairaaloihin tai mihin tahansa, missä työpöytätila
    on kortilla, mutta suorituskyvystä ei haluta tinkiä.
  keywords: Dell pöytäkone, pöytäkoneet dell, dell micro sarja, dell pöytäkoneet
    yrityskäyttöön, tietokoneet pk -yritykselle
breadcrumb:
  - path: /
    label: TDP
  - path: /it-laitteet/
    label: IT-laitteet
  - path: /it-laitteet/dell-mallisarjat/
    label: Dell mallisarjat
  - path: /it-laitteet/dellpromicro_desktop/
    label: Pro Micro
contactForm: Ota yhteyttä
---

<HeroBlock bgColor="brand" imageAlign="right">

<div className="HeroBlockImage">

![Työntekijöille yrityskannettavat leasing-sopimuksella](../../static/assets/tdp-palvelukumppani.png)

</div>

<div className="HeroBlockContent">

# Yritysten IT-laitteet

* Vakioidut ja yhteensopivat laitemallit käyttökuntoon asennettuina 
* Leasing tai suoraosto yrityksellesi sopivalla mallilla
* Kaikki yleisimmät laitemerkit - kilpailuta meidät
* Nopea apu ja arvio myös puhelimitse, soita <DisplayVariable variableKey="myynnin-puhelin" bold={true} />

*Tarvitsetko arvion IT-kokonaisuuden kustannuksista?*

<CallToAction bgColor="dark" url="/it-laitteet/arviolaskuri/" align="left">Kokeile laskuria</CallToAction>

<CallToAction bgColor="dark" url="#contact-form" align="left">Jätä tarjouspyyntö</CallToAction>

</div>

</HeroBlock>


# Dell Pro Micro


**Dell Pro Micro -sarja on suunniteltu moderneihin, tilatehokkaisiin työympäristöihin. Se on täydellinen valinta avokonttoreihin, asiakaspalvelupisteisiin, sairaaloihin tai mihin tahansa, missä työpöytätila on kortilla, mutta suorituskyvystä ei haluta tinkiä.**



![Kuvituskuva: Dell Pro Micro](../../static/assets/kuva-dell-micro-desktop2-600x450-.jpg "Dell Pro Micro sarjan pöytätietokoneet yrityksille")

Tämä sarja todistaa, ettei suuri teho vaadi suurta koteloa. Vain **1,18 litran** kokoinen runko kätkee sisäänsä täysiverisen työkoneen. Yritysjohdolle sarja tarjoaa joustavuutta ja siisteyttä: innovatiivinen **yhden kaapelin ratkaisu** mahdollistaa laitteen virransyötön ja kuvan siirron yhdellä USB-C-kaapelilla yhteensopivasta näytöstä, mikä vähentää johtosotkua merkittävästi.

- - -

### **Tekoälyvalmis suorituskyky ja hiljainen toiminta**

Pienestä koostaan huolimatta Pro Micro -sarja on varustettu uusimmalla teknologialla, joka tehostaa työskentelyä ja vähentää melua.

* **Prosessori ja NPU:** Laitteet hyödyntävät Intel® Core™ Ultra -prosessoreita (Series 2), joissa on integroitu NPU (Neural Processing Unit). Tämä mahdollistaa tekoälyominaisuuksien, kuten videoneuvottelujen taustan sumennuksen ja äänen parannuksen, ajamisen energiatehokkaasti. (Pro Micro -malliin saatavilla myös Intel Core 14. sukupolven suorittimet).  
* **Hiljaisuus:** Uudistettu lämpösuunnittelu tekee laitteesta jopa **32 % hiljaisemman** ja viileämmän kuin aiemmat sukupolvet, mikä parantaa työviihtyvyyttä.  
* **Muisti ja Tallennus:** Jopa 64 GB nopeaa DDR5-muistia ja kaksi M.2 SSD -paikkaa takaavat, että sovellukset käynnistyvät viiveettä.

### **Joustava liitettävyys ja kiinnitysratkaisut**

**Dell Pro Micro** mukautuu mihin tahansa työpisteeseen modulaarisuutensa ansiosta.

* **Kiinnitys:** Laaja valikoima telineitä mahdollistaa laitteen kiinnittämisen näytön taakse, pöydän alle tai integroituna All-in-One-jalustaan.  
* **Laajennettavuus:** Valinnainen moduuliportti mahdollistaa takapaneelin räätälöinnin: voit lisätä esimerkiksi  HDMI 2.1:n, VGA:n tai jopa kuituyhteyden.  
* **Moninäyttötuki:** Tukee jopa neljää näyttöä natiivisti tai ketjuttamalla (Daisy Chain).

### **Kestävä kehitys ja tietoturva**

Laite on rakennettu kestävän kehityksen periaatteilla ja se on yksi markkinoiden turvallisimmista.

* **Materiaalit:** Runko sisältää jopa 50 % kierrätettyä terästä ja tuulettimen kotelo on valmistettu merten muovijätteestä. Pakkaus on 100 % kierrätettävää tai uusiutuvaa materiaalia.  
* **Tietoturva:** Dell Trusted Workspace tarjoaa BIOS-tason suojauksen ja toimitusketjun eheyden varmistuksen, suojaten laitetta hyökkäyksiltä ensikäynnistyksestä alkaen.

- - -

### **Tekniset tiedot**

|                         |                                                                |
| ----------------------- | -------------------------------------------------------------- |
| **Suoritin**            | Intel® Core™ Ultra (Series 2) tai Intel® Core™ (14. sukupolvi) |
| **Keskusmuisti**        | Jopa 64 GB DDR5                                                |
| **Tallennustila**       | 2 x M.2 SSD (Maks. 1 TB / levy)                                |
| **Videolähdöt (vakio)** | 3 x DisplayPort 1.4a                                           |
| **USB-portit**          | 4 kpl (sis. 1 x USB-C edessä)                                  |
| **Valinnainen moduuli** | HDMI, DP, VGA, Serial, USB-C                                   |
| **Mitat**               | 182 x 36 x 178 mm (1.18 L)                                     |
| **Virtalähde**          | Ulkoinen AC-adapteri tai USB-C PD (jopa 100W)                  |

- - -

Dell Pro Micro on tilaa säästävä tehopakkaus, joka ei tingi ominaisuuksista. Hanki laitteet meiltä esiasennettuna ja integroituna yrityksesi hallintajärjestelmiin.
